...
1. **Use the Task tool** to invoke the git-droid sub-agent:
   ...
   - **prompt:** "Initialize the launch workflow with the following parameters: [pass all user arguments and auto mode]. Auto mode: [resolved auto mode value]. You must:
     ...
     - Handle scenarios based on auto mode:
       * If uncommitted changes detected:
         - If auto mode is TRUE: Automatically choose option 1 (move changes to feature branch) and set 'Next Stage: HANDLE_CHANGES'
         - If auto mode is FALSE: Present numbered options:
           1. Move changes to feature branch [RECOMMENDED]
           2. Discard changes
           3. Abort launch
       * If existing session detected (no uncommitted changes):
         - If auto mode is TRUE: Automatically choose option 1 (abort existing session) and set 'Next Stage: HANDLE_SESSION'
         - If auto mode is FALSE: Present numbered options:
           1. Abort existing session [RECOMMENDED]
           2. Keep both sessions
           3. Abort launch
       * If both issues detected:
         - If auto mode is TRUE: Automatically choose to move changes and abort session, set 'Next Stage: HANDLE_BOTH'
         - If auto mode is FALSE: Present combined options handling both
     ...
     - IMPORTANT: Create workflow state object and include it in your response:
       ```json
       {
         \"workflowId\": \"launch-[timestamp]\",
         \"stage\": \"STAGE_1\",
         \"verified\": {
           \"onMainBranch\": [true/false],
           \"mainSynced\": [true/false],
           \"changesChecked\": true,
           \"sessionChecked\": true
         },
         \"context\": {
           \"hasUncommittedChanges\": [true/false],
           \"hasExistingSession\": [true/false],
           \"userChoice\": \"[HANDLE_CHANGES/HANDLE_SESSION/HANDLE_BOTH/CREATE_BRANCH/ABORTED]\",
           \"moveChangesToFeature\": [true/false if applicable],
           \"autoModeUsed\": [true/false]
         }
       }
       ```
     - IMPORTANT: In your Result Summary section, include EXACTLY one of:
       * 'Next Stage: HANDLE_CHANGES' (auto mode chose option 1, or user chose option 1 for uncommitted changes)
       * 'Next Stage: HANDLE_SESSION' (auto mode chose option 1, or user chose option 1 for existing session)
       * 'Next Stage: HANDLE_BOTH' (auto mode chose to handle both, or user chose to handle both)
...
4. **Check the response** for the "Next Stage:" directive in Result Summary:
   - If 'Next Stage: HANDLE_CHANGES', proceed to Stage 2 (Handle Uncommitted Changes) with workflow state
   - If 'Next Stage: HANDLE_SESSION', skip to Stage 3 (Handle Existing Session) with workflow state
   - If 'Next Stage: HANDLE_BOTH', proceed to Stage 2 then Stage 3 with workflow state
   - If 'Next Stage: CREATE_BRANCH', skip to Stage 4 (Create Feature Branch) with workflow state
...
1. **Use the Task tool** to invoke the git-droid sub-agent:
   ...
   - **prompt:** "Handle uncommitted changes based on user's choice from Stage 1, using workflow state from Stage 1: [pass workflow state object]. You must:
     - SKIP branch verification (already verified in Stage 1 - check workflow state)
     - SKIP main sync check (already checked in Stage 1 - check workflow state)
     ...
     - Update workflow state object:
       ```json
       {
         ...
         \"context\": {
           ...
           \"changesHandled\": [true/false],
           \"changeAction\": \"[stashed/discarded]\"
         }
       }
       ```
     - Format results following git-droid output style from `.claude/output-styles/git-droid.md`
...
4. **Check the response** for the "Next Stage:" directive in Result Summary:
   - If 'Next Stage: HANDLE_SESSION', proceed to Stage 3 (Handle Existing Session) with workflow state
   - If 'Next Stage: CREATE_BRANCH', skip to Stage 4 (Create Feature Branch) with workflow state
   - If 'Next Stage: ABORTED', terminate workflow
...
1. **Use the Task tool** to invoke the git-droid sub-agent:
   ...
   - **prompt:** "Handle existing session based on user's choice from Stage 1, using workflow state from previous stages: [pass workflow state object]. You must:
     - SKIP branch verification (already verified in Stage 1 - check workflow state)
     - SKIP main sync check (already checked in Stage 1 - check workflow state)
     ...
     - Update workflow state object:
       ```json
       {
         ...
         \"context\": {
           ...previous context fields...,
           \"sessionHandled\": [true/false],
           \"sessionAction\": \"[aborted/kept-both]\"
         }
       }
       ```
     - Format results following git-droid output style from `.claude/output-styles/git-droid.md`
...
4. **Check the response** for the "Next Stage:" directive in Result Summary:
   - If 'Next Stage: CREATE_BRANCH', proceed to Stage 4 (Create Feature Branch) with workflow state
   - If 'Next Stage: ABORTED', terminate workflow
...
1. **Use the Task tool** to invoke the git-droid sub-agent:
   ...
   - **prompt:** "Create the feature branch with the following parameters: [pass all user arguments] and workflow state from previous stages: [pass workflow state object]. You must:
     - SKIP branch verification (already verified in Stage 1 - check workflow state)
     - SKIP main sync check (already checked in Stage 1 - check workflow state)
     - SKIP uncommitted changes check (already handled in Stage 2 if needed - check workflow state)
     - SKIP session conflict check (already handled in Stage 3 if needed - check workflow state)
...
- **Stage 1** performs all pre-flight checks (branch, sync, changes, session) and creates workflow state object
- **Stage 2** (conditional) receives state, skips branch/sync checks, handles uncommitted changes
- **Stage 3** (conditional) receives state, skips all prior checks, handles existing session
- **Stage 4** receives state, skips all prior verifications, creates feature branch

Each stage updates the state object with new context before passing to the next stage. This ensures:
- No redundant branch verification
- No redundant main sync checks
